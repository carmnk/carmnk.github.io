...
export var addSubchart = function (current, params) {
    ...
    var style = getDefaultGraphStyle(data.type, current.options.isDarkMode, undefined, indicatorLines);
    var dataId = data.id;
    if (current.subCharts.length === 0 || !!reset) {
        var top_1 = 0;
        var bottom = containerHeight - current.options.xaxis.heightXAxis;
        ...
        var subchartsHeight = containerHeight - current.options.xaxis.heightXAxis;
        var subCharts = resizeSubcharts({
            subchartsHeight: subchartsHeight,
            subCharts: current.subCharts,
            addSubchart: { data: data, darkMode: current.options.isDarkMode },
        });
...
export var removeSubchart = function (current, params) {
    ...
    var subchartsHeight = current.containerSize.height - current.options.xaxis.heightXAxis;
    var subCharts = resizeSubcharts({
        subchartsHeight: subchartsHeight,
        subCharts: current.subCharts,
...
export var addGraph = function (current, params) {
    ...
    var getStrokeColor = function (idx) {
        return getGraphColors(current.options.isDarkMode ? graphColorsDark : graphColorsLight, idx);
    };
    ...
    var dataId = data.id;
    var indicatorLines = data.type === "indicator" ? data.indicator.graphTypes.length : undefined;
    var style = getDefaultGraphStyle(data.type, current.options.isDarkMode, newGraphIdx, indicatorLines);
...
export var addInitialData = function (current, params) {
    ...
    var initSubcharts = inputData ? getInitSubchartsState(current.options.isDarkMode, inputData) : [];
...
export var clearChart = function (current, params) {
    ...
    var clearedSubcharts = current.subCharts
        ...
        return mode === "tools" || sIdx === 0
            ? __assign(__assign({}, sub), { bottom: mode !== "tools" ? current.containerSize.height - 1 - current.options.xaxis.heightXAxis : sub.bottom, yaxis: sub.yaxis.map(function (yax) {
                    var _a;
