...
export var createYaxisModel = function (params) {
    ...
    if (type === "indicator" && !indicator)
        return null;
    var graph = type === "indicator" && !!indicator
...
export var swapSubcharts = function (current, params) {
    ...
    return lowerIdx === higherIdx || isNullish(lowerIdx) || isNullish(higherIdx) || higherIdx >= current.subcharts.length
        ...
        : __assign(__assign({}, current), { subcharts: __spreadArray(__spreadArray(__spreadArray(__spreadArray(__spreadArray([], current.subcharts.slice(0, lowerIdx), true), [
                ...
                .map(function (sub) { return (__assign(__assign({}, sub), { top: sub.top + dHeight21, bottom: sub.bottom + dHeight21 })); }), true), [
...
export var resizeSubcharts = function (params) {
    ...
    if (subchartsHeight >= optHeight) {
        ...
        var sumShrinkedSecSubHeights_1 = shrinkedSecondarySubHeights_1.reduce(function (acc, cur) { return acc + cur; }, 0);
        var resizedSubcharts = subchartsDeleted.reduce(function (acc, cur, idx) {
            return idx === 0
                ? [
...
export var modifyToolProp = function (current, params) {
    var newValue = params.newValue, prop = params.prop, subchartIdx = params.subchartIdx, yaxisIdx = params.yaxisIdx, toolIdx = params.toolIdx, toolParamIdx = params.toolParamIdx;
...
export var modifyGraphProp = function (current, params) {
    var newValue = params.newValue, prop = params.prop, subchartIdx = params.subchartIdx, yaxisIdx = params.yaxisIdx, graphIdx = params.graphIdx;
...
export var removeTool = function (current, params) {
    var subchartIdx = params.subchartIdx, yaxisIdx = params.yaxisIdx, toolIdx = params.toolIdx;
