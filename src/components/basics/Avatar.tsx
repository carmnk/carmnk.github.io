...
export type CAvatarProps = Omit<AvatarProps, 'title'> & {
  size?: number
  person?: { first_name?: string; last_name?: string; title?: string | null; company_name?: string }
  fontSize?: any
  disableInitials?: boolean
  title?: React.ReactNode
}
export const CAvatar = (props: CAvatarProps) => {
  const { size = 26, src, children, person, style, fontSize, disableInitials, title, ...rest } = props
  ...
  return src ? (
    <div className="flex items-center justify-start">
      <Tooltip
        placement="top"
        arrow
        title={title ?? formatFullName(person?.title, person?.first_name, person?.last_name)}
      >
        <Box
          component="img"
          style={{ width: size, height: `max(${size}, 100%)`, borderRadius: 99999, ...style }}
          src={src}
          alt="profile"
          {...rest}
        />
